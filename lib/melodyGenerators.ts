    ...
    restProbability = 0.2
      ...
      function generateRhythmWithRests(
          ...
          while (remainingNotes > 0 && remainingBeats >= absoluteMinDurationValue - tolerance) {
              ...
              const minSpaceNeeded = getMinSpaceNeededForNotes(remainingNotes, minValue);
              
              if (remainingBeats < minSpaceNeeded - tolerance) {
                  break;
              }
  
              const maxPossibleValue = Math.min(
                  maxValue,
                  remainingBeats - (remainingNotes - 1) * minValue,
                  remainingBeats
              );
              ...
              const canAddRest = remainingBeats - durationToUse.value >= minSpaceNeeded;
              const isRest: boolean = canAddRest && Math.random() < restProb && localResult.length > 0;
